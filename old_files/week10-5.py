...
def main():
	with open('./src/wifi_passive_trajectory_node.csv', newline='') as csvfile:
		...
		# AGE( jwalkReader )
		findNegative( jwalkReader )

def main2():
	readNodes()

def findNegative( reader ):
	for row in reader:
		nodes = json.loads( row[3] )
		for node in nodes:
			if int(node['duration']) < 0:
				print(node)
				break
...
def readNodes():
	...
	query = df[ (df.take_time < 0) | (df.duration < 0) ]
	print( query )
...
def exportStarttime():
	...
	j = list(df['starttime'])
	with open('./src/nodes.json', 'r+') as f:
		writeString = ""
		f.write('[')
		for nodes in j:
	...
	df2 = pd.read_json( './src/nodes.json' )
	print( df2.describe() )
