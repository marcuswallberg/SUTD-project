...
src = './src/wifi_passive_trajectory_node.csv'
jsonSrc = './src/nodes.json'
...
def main():
	with open('./src/wifi_passive_trajectory_node.csv', newline='') as csvfile:
		...
		# getAverageBridgeTime( jwalkReader )
		# InsideOutsideInside( jwalkReader )
		# OutsideInsideOutside( jwalkReader )
		# AGE( jwalkReader )
		printNormalSpikes2( jwalkReader )
...
def jsonToCsv( jsonSrc = jsonSrc, dest = './src/nodes.csv' ):
...
def exportStarttime():
	...
	j = list(df['starttime'])
	with open('./src/nodes2.json', 'r+') as f:
		writeString = ""
		f.write('[')
		for nodes in j:
	...
	df2 = pd.read_json( './src/nodes2.json' )
	print( df2.describe() )
		...
		for c in current:
			...
			if (staynode in hospitalExit[0] or staynode in hospitalExit[1]) and insideOfHospital:
				if (staynode in hospitalExit[0] and enterHospital) or (staynode in hospitalExit[1] and not enterHospital):
					...
					staynode = ""
					if duration > 1000 and duration <= 10000:
						if dayType < 5:
							weekday.append(t)
						else:
							weekend.append(t)
	...
	for row in reader:
		...
		for c in current:
			...
			if staynode in notHostpitals and insideOfHospital:
				...
				t = int( datetime.fromtimestamp( int(c['starttime']) ).hour )
				staynode = ""
				if duration >= 700 and duration < 1000:
					if dayType < 5:
						weekday.append(t)
					else:
						weekend.append(t)
