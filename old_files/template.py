# ROW: ['_id', 'created_on', 'date', 'starttime', 'user_id']
# PATH: starttime: [{"stay_node","endtime","starttime","duration","next_node","take_time"}]
import json
import csv
import numpy as np
...
"""
Varje person förekommer endast 1 gång per dag. Alla PATH ligger i en ROW per dag

"""

# Spara lite globala variabler här:
...
def main():
	with open('./src/wifi_passive_trajectory_node.csv', newline='') as csvfile:
		...
		# Kalla på dina funktioner här:

		checkForBackAndForth( jwalkReader )

# Den här funktionen kollar om man går till en nod och går tillbaka direkt
# Den printar antal gånger som det händer
def checkForBackAndForth( reader ):
	count = 0

	# Gå igenom varje rad data
	# ROW: ['_id', 'created_on', 'date', 'starttime', 'user_id']
	# PATH: starttime: [{"stay_node","endtime","starttime","duration","next_node","take_time"}]
	for row in reader:
		# Hämtar all data
		_id, created_on, _date, paths, starttime, user_id = row
		paths = json.loads( paths )	# Hämtar hela listan
	...
	print( "Amout of times a person goes back to the previous node:", count )
